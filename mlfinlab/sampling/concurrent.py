...
def num_concurrent_events(close_series_index, label_endtime, molecule):
    ...
    label_endtime = label_endtime[
        label_endtime >=
        molecule[0]]  # Events that end at or after molecule[0]
    ...
    nearest_index = close_series_index.searchsorted(
        pd.DatetimeIndex([label_endtime.index[0],
                          label_endtime.max()]))
    count = pd.Series(
...
def get_av_uniqueness_from_triple_barrier(triple_barrier_events, close_series,
                                          num_threads):
    """
